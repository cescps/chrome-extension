...
function getSettings() {
  return new Promise(resolve =>
    chrome.storage.sync.get(['deepseekApiKey'], d => resolve({
      apiKey: d.deepseekApiKey || null
    }))
  );
}
...
function isTooSimilar(candidateText, pool) {
  ...
  for (const posted of pool) {
    ...
    // Jaccard similarity (lowered threshold to 0.65)
...
async function rewriteAndPost(tweetText, originalText) {
  ...
  setStatus('Rewriting…');
  const rewritten = await callDeepSeek(tweetText, apiKey);
  ...
  if (whoopsEl) {
    ...
    // Wait for "Save post?" modal to appear, then click Discard
    let discardBtn = null;
    for (let i = 0; i < 10; i++) {
      await sleep(300);
...
async function runLoop() {
  while (running) {
    ...
    for (const article of candidates) {
      ...
      // Similarity check before rewriting
...
// Re-attach overlay if X.com's React wipes it, and refresh relative times
