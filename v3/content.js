...
async function rewriteAndPost(tweetText, originalText) {
  ...
  // Always clear the editor before pasting — prevents leftover text from prior attempts
  document.execCommand('selectAll', false, null);
  await sleep(50);
  document.execCommand('delete', false, null);
  await sleep(100);

  // Paste using a fresh DataTransfer (does NOT rely on system clipboard)
  ...
  // Verify the editor actually has the correct text
  ...
  if (editorNorm !== rewrittenNorm) {
    ...
    // If still wrong after retry, bail out and clean up
